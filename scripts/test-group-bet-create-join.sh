#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
#  test-group-bet-create-join.sh — runner for gp-1-02 (Day 2)
#  ═══════════════════════════════════════════════════════════════
#  Picks 3 ACTIVE users from the live DB, promotes them to kyc_tier=1
#  so the group-bet gate passes, and runs the create/join integration
#  test against the live Postgres.
#
#  Cleanup:
#    - Restores original kyc_tier at end
#    - Deletes test data via the test's own cleanup() function
#
...
# ── 4. Run the test ─────────────────────────────────────────────
# Override REDIS_HOST to 127.0.0.1 so the test can use the host-side
# socat forwarder (the .env has REDIS_HOST=redis, which is unreachable
# from the host's bare ts-node process).
export REDIS_HOST=127.0.0.1
export REDIS_PORT=6379
cd "$ROOT/backend"
set +e
exec npx ts-node ./src/test/gp-1-02-group-bet-create-join.test.ts
RC=$?

# Restore creator kyc_tier regardless of test outcome
"${PSQL[@]}" -c "UPDATE users SET kyc_tier='$CREATOR_KYC' WHERE id = '$CREATOR_ID'" > /dev/null 2>&1 || true
# Restore the two members (don't know if they had kyc; default to 0)
