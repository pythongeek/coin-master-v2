#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
#  test-group-bet-config.sh — runner for gp-2-01 (Day 8 Phase 2)
#  ════════════════════════════════════════════════════════
#
...
# Promote creator to KYC tier 1 + ensure $50 lifetime deposits
docker exec coin-master-postgres-1 psql -U "$USER_" -d "$DB" >/dev/null 2>&1 <<EOF
UPDATE users SET kyc_tier='1' WHERE id = '$CREATOR' AND kyc_tier::int < 1;
...
echo "[run] CREATOR=$CREATOR  (kyc promoted to tier 1)"

# ── 4. Run the test ─────────────────────────────────────────────
cd "$ROOT/backend"
exec npx ts-node ./src/test/gp-2-01-group-config.test.ts