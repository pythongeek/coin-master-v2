...
import { BarChart3, Settings, Users, ShieldCheck, Gift, Wallet, FileText, Key, Activity, LogOut, AlertCircle, AlertOctagon, Trophy, Target, Coins, QrCode, Mail, FlaskConical, SlidersHorizontal, Brain, Globe2, type LucideIcon } from 'lucide-react';
...
import AdminGeoipSettings from '@/components/dashboard/AdminGeoipSettings';
...
import AdminBalanceAdjustment from '@/components/dashboard/AdminBalanceAdjustment';
...
const TABS: { id: 'live' | 'config' | 'users' | 'withdrawals' | 'deposits' | 'bonuses' | 'leaderboard' | 'challenges' | 'audit' | 'system_logs' | 'health' | 'email' | 'security' | 'kyc' | 'kyc_admin' | 'account' | 'coins' | 'fraud' | 'testing' | 'settings' | 'ml' | 'audience' | 'geoip'; label: string; Icon: LucideIcon }[] = [
  ...
  { id: 'geoip', label: 'GeoIP Settings', Icon: Globe2 },
...
export default function AdminClientShell({ user }: AdminClientShellProps) {
  ...
  return (
    <main className="min-h-screen flex flex-col">
      {/* Header — unchanged */}
      <header className="p-4 md:p-6 border-b border-border bg-surface/80 backdrop-blur-sm">
      ...
      {/* 2FA warning — unchanged */}
      {needs2FA && (
        <div className="mx-4 md:mx-6 mt-4 flex items-center gap-3 px-4 py-3 rounded-lg border border-brand-gold/40 bg-brand-gold/10 text-brand-gold text-xs font-mono">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left sidebar menu */}
        <aside className="w-64 shrink-0 border-r border-border bg-surface/60 p-4 overflow-y-auto">
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-mono text-left transition-all duration-150 ${
                    activeTab === tab.id
        ...
        <section className="flex-1 overflow-y-auto p-4 md:p-6">
          <div className="space-y-5">
            ...
            {activeTab === 'geoip' && <AdminGeoipSettings />}
            ...
            {activeTab === 'kyc_admin' && isSuperAdmin && (
              <AdminKycOverrides />
            )}
            {activeTab === 'audit' && <AdminAuditLogViewer />}
            {activeTab === 'system_logs' && <AdminSystemLogs />}
