...
- [x] **[GP-16] Group Play — Gap 16: country enforcement on create + join (closes sanctioned-country bypass)** ✓ TESTED & PASSED 2026-08-04
  ...
  - **Status**: `[x] [TESTED & PASSED 2026-08-04]` (Gap 16)

- [x] **[GP-15] Group Play — Gap 15: every admin-group POST route writes to admin_actions (closes the central-audit gap)** ✓ TESTED & PASSED 2026-08-04
  - **File(s) Affected**: `backend/migrations/049_group_admin_action_types.sql` (NEW, 10 lines — 7 `ALTER TYPE "AdminActionType" ADD VALUE IF NOT EXISTS` for `group_force_cancel`, `group_freeze`, `group_unfreeze`, `group_mark_fraud`, `group_refund`, `group_kick`, `group_shadow`); `backend/src/routes/admin-groups.ts` (+60, ~1 — added the `recordGroupAdminAction({groupId, adminId, action, reason, metadata, ipAddress})` helper that writes `admin_actions` with `id=uuid_generate_v4()`, `target_type='group_bet'`, `approval_status='approved'`, `approved_by_id=admin_id`, `approved_at=NOW()`, `executed_at=NOW()`, `ip_address=req.ip`, and `new_value` as the metadata jsonb; wired the helper into all 6 POST routes: `/force-cancel`, `/freeze`, `/mark-fraud`, `/refund`, `/kick/:userId`, `/shadow`)
  - **Issue/Gap**: The 6 admin-group POST routes (`/:id/force-cancel`, `/:id/freeze`, `/:id/mark-fraud`, `/:id/refund`, `/:id/kick/:userId`, `/:id/shadow`) already wrote to `audit_log` (via `recordAdminForce`) and `group_bet_audit` (inline INSERTs), but never to `admin_actions` — the table the central admin-audit dashboard pivots off. The `AdminActionType` enum had 6 values (`balance_adjustment`, `rate_override`, `seed_rotation`, `config_change`, `withdrawal_approval`, `kyc_manual_review`) but no `group_*` values, so even if a route had tried to write the row, the enum would have rejected it. Admin operators could not search the admin_actions timeline for "who cancelled this group?" or "who froze this room?" — only the more granular `audit_log` and `group_bet_audit` (which the admin UI does not aggregate).
  - **Proposed Fix**:
    1. Migration 049 extends `AdminActionType` with 7 new values covering every admin-group operation. The freeze toggle uses `group_freeze` / `group_unfreeze` based on the resulting `is_frozen` state.
    2. `recordGroupAdminAction` helper: stable, typed-in-TypeScript surface, single SQL INSERT, wrapped in try/catch so an audit-write failure does NOT roll back the operator action (the existing `audit_log` + `group_bet_audit` rows still stand).
    3. One call per route, placed after the existing `recordAdminForce` audit signal so the operational action ALWAYS writes as a paired pair (admin_force + admin_actions, same timestamp window).
  - **Live verification (post-deploy, 2026-08-04)**:
    | Scenario | Expected | Got |
    |---|---|---|
    | `POST /api/admin/groups/<id>/force-cancel` | 200 + admin_actions row with action_type='group_force_cancel' | ✅ 1 row, target_type='group_bet', justification='gap15 verification smoke test', approval_status='approved', ip_address set, new_value={'refundedMembers':1,'refundedTotal':'1.00000000','transitioned':true} |
    | `POST /api/admin/groups/<id>/freeze` (first call) | 200 + admin_actions row with action_type='group_freeze' | ✅ 1 row, new_value={'is_frozen':true} |
    | Re-call freeze (toggle) | 200 + admin_actions row with action_type='group_unfreeze' | ✅ 1 row, new_value={'is_frozen':false} |
    | `POST /api/admin/groups/<id>/mark-fraud` | 200 + admin_actions row with action_type='group_mark_fraud' | ✅ 1 row, new_value={'signalType':'group_admin_force','severity':'high','frozen':true} |
    | `POST /api/admin/groups/<id>/refund` (resolved room) | 200 + admin_actions row with action_type='group_refund' | ✅ 1 row, new_value={'reversedTotal':'2.00000000','reversedWinners':1} |
    | `POST /api/admin/groups/<id>/kick/:userId` | 200 + admin_actions row with action_type='group_kick' | ✅ 1 row, new_value={'kickedUserId':userId,'refunded':'1.00000000','wasCreator':true} |
    | `POST /api/admin/groups/<id>/shadow` | 200 + admin_actions row with action_type='group_shadow' | ✅ 1 row, new_value={'shadowed':true} |
    | Final `admin_actions` summary | 7 distinct action_type values populated | ✅ group_force_cancel:2, group_freeze:3, group_unfreeze:1, group_mark_fraud:1, group_refund:1, group_kick:2, group_shadow:2 |
  - **Pre-existing bug also fixed (incidental)**:
    - `mark-fraud` route hit a 500 error because the original `fraud_signals INSERT ... ON CONFLICT (fingerprint)` requires a unique constraint on `fingerprint`, but `fraud_signals.fingerprint` has NO unique constraint (verified in pg_constraint). This blocked the operator action entirely. Fixed by replacing the `ON CONFLICT` clause with a plain INSERT wrapped in try/catch (duplicate-key error 23505 is swallowed). The mark-fraud admin call now succeeds and the admin_actions row is recorded.
  - **Live verification environment**: `k6test_0` (BD country, creator) + `admin` (super_admin) login → group created via JWT-authed POST → admin route called with `ChangeMe!Admin2026` password → db queries via `docker exec coin-master-postgres-1 psql`. Test groups were force-cancelled/cancelled at end of run for cleanup.
  - **SPEC VERIFICATION flip**: Group admin operations are now first-class entries in `admin_actions` with type-safe `AdminActionType` enum values, full IP capture, and structured jsonb metadata. The central admin-actions dashboard (which already aggregates `balance_adjustment`, `rate_override`, etc.) now also surfaces `group_*` events alongside the existing rows. Admin operators can finally search the unified timeline for "who cancelled group X at time Y" without joining to `audit_log`.
  - **Status**: `[x] [TESTED & PASSED 2026-08-04]` (Gap 15)

- [x] **[GP-9] Group Play — Gap 9: house-side accounting triple in `ledger_entries` (pool_received / pool_paid_out / house_take)** ✓ TESTED & PASSED 2026-08-04
  - **File(s) Affected**: `backend/migrations/050_group_house_ledger.sql` (NEW, ~80 lines — 3 `ALTER TYPE "LedgerEntryType" ADD VALUE IF NOT EXISTS` for `group_pool_received`, `group_pool_paid_out`, `group_house_take`; INSERT a USD currency row with `code='USD'`, `is_default=true`, `exchange_rate=1.0` because the `currencies` table was empty; INSERT a `_house` sentinel user with the fixed UUID `00000000-0000-0000-0000-000000000001` because `ledger_entries.user_id` is NOT NULL but house-side entries are inherently userless). All inserts use `ON CONFLICT DO NOTHING` for idempotency. `backend/src/services/group-bet-flip.ts` (+60 — new step 7c.1 in the `withTransaction` block of `flipGroup()` that INSERTs 3 `ledger_entries` rows after the per-member payout `transactions` and before the audit mirror, all inside the same SERIALIZABLE TX so the user-side and house-side legs commit atomically).
  - **Issue/Gap**: The `transactions` table recorded the user-side legs of a group bet (debit at JOIN, credit at win, `admin_adjustment` at cancel/refund) but the **house-side accounting** was completely invisible to the ledger system. The `ledger_entries` table (a separate, hash-chained, ref-unique ledger used by the accounting dashboard and `wallet.service.ts` aggregations) was empty for group play. An operator auditing the books could see members receiving $X in payouts but had no record of how much the pool was, how much was paid out, and how much the house took. The `LedgerEntryType` enum had 10 values (`deposit`, `withdrawal`, `bet`, `win`, `loss`, `refund`, `admin_adjustment`, `fee`, `transfer_in`, `transfer_out`) but no `group_*` values, so the ledger literally could not accept these entries.
  - **Proposed Fix**:
    1. Extends `LedgerEntryType` with 3 new values: `group_pool_received`, `group_pool_paid_out`, `group_house_take`.
    2. Seeds a USD currency row (the `currencies` table was EMPTY on the live DB — every previous insert into `ledger_entries` would have failed not-null on `currency_id`).
    3. Seeds a `_house` sentinel user (`is_active=false`, `is_admin=false`, `kyc_status='unverified'`, no password) at fixed UUID `00000000-0000-0000-0000-000000000001` so the `user_id` NOT NULL constraint on `ledger_entries` can be satisfied for house-side rows. The sentinel is never queryable through the auth surface.
    4. Code change: in `flipGroup()`, after the per-member `transactions` win INSERTs (step 7c) and inside the same `withTransaction` block, compute `totalPaidOut = sum(payouts)` and `houseTake = max(totalPool - totalPaidOut, 0)`, then INSERT 3 `ledger_entries` rows with `entry_type` ∈ {group_pool_received, group_pool_paid_out, group_house_take}, `user_id` = `_house` sentinel, `currency_id` = USD, `amount` = totalPool / totalPaidOut / houseTake, `reference_id` = `group-${id}-${entry_type}` (unique per row via the existing UNIQUE index on `reference_id`), `metadata` = {ref_type:'group_bet', ref_id:<id>, …per-row enrichment (memberCount, payoutMode, winnerCount, houseEdgePct, totalPool, totalPaidOut, winningSide)}.
  - **Live verification (post-deploy, 2026-08-04)**:
    | Scenario | Expected | Got |
    |---|---|---|
    | Setup: enable `group_play`, create group k6test_0 (creator) + k6test_19 (member), join, flip | 200 + 3 ledger_entries rows | ✅ all 3 written |
    | Win case (winningSide='heads', both members picked heads) | pool_received=2.0, pool_paid_out=2.0, house_take=0.0 | ✅ exactly that — invariant: 2.0 = 2.0 + 0.0 |
    | Loss case (winningSide='tails', both members picked heads) | pool_received=2.0, pool_paid_out=0.0, house_take=2.0 | ✅ exactly that — invariant: 2.0 = 0.0 + 2.0 |
    | 4 flips total (mixed) | invariant holds across all groups | ✅ aggregate: pool_received=8.0, pool_paid_out=4.0, house_take=4.0; 8.0 = 4.0 + 4.0 |
    | `metadata->>'ref_id' = <groupId>` query | 3 rows per resolved group | ✅ confirmed |
    | `metadata->>'ref_type' = 'group_bet'` | present | ✅ confirmed |
    | `_house` sentinel user | exists, `is_active=false`, never logged in | ✅ confirmed |
    | `USD` currency row | exists, `is_default=true`, `exchange_rate=1.0` | ✅ confirmed |
  - **Coverage notes (audit prompt: "on all paths?")**:
    1. **Win path** (resolve after flip, all members on winning side): 3 rows written ✅
    2. **Loss path** (resolve after flip, members on losing side): 3 rows written ✅ (pool_paid_out=0, house_take=totalPool)
    3. **Cancel path** (`POST /:id/force-cancel`): 0 rows in `ledger_entries` — this path uses `creditPayout` + `transactions` (admin_adjustment with `reason='group_bet_admin_force_cancel'`) to refund members. The pool is reversed to members, so no house-side take is generated. The audit trail is in `audit_log` + `group_bet_audit` + `admin_actions` (Gap 15). **Coverage decision:** correct — force-cancel is a money-back operation, not a winning outcome.
    4. **Refund path** (`POST /:id/refund` on `resolved` room): 0 rows in `ledger_entries` — same rationale. Refunds debit winners' balances via `transactions` (admin_adjustment with `reason='group_bet_admin_refund'`). The original flip's 3 `ledger_entries` rows are preserved as the audit trail of the original resolve. **Coverage decision:** correct — refund is a money-back operation that reverses the user-side leg, not a new house-side event.
    5. **Expiry path** (Day 4 sweep, `group_bet_expiry.ts`): reuses the same `creditPayout` + `transactions` flow, so same as cancel. **Coverage decision:** correct.
  - **Pre-existing observation (not fixed in Gap 9)**: `computePayouts()` does NOT subtract the 2% house edge from the winners' payout — it returns the full `totalPool / winners.length`. So when all members win, `house_take=0`. The `houseEdgePct` is passed to `resolveFlip()` but never threaded into `computePayouts()`. This is a pre-existing accuracy bug, not a Gap 9 issue. The ledger correctly records what the code actually computes, and the invariant `pool_received = pool_paid_out + house_take` holds in both states. Tracked as a follow-up for a separate gap.
...
- [x] **[GP-4] Group Play — Gap 4: bonus wagering credits groups at admin-configurable weight (default 50%)** ✓ TESTED & PASSED 2026-08-04
  - **File(s) Affected**: `backend/migrations/052_seed_group_bonus_wager_weight.sql` (NEW ~25 lines — INSERT `admin_settings.group_bonus_wager_weight='50'` with `ON CONFLICT (key) DO NOTHING`); `backend/src/services/admin-group-config.ts` (+~15 lines — added `groupBonusWagerWeight: number` to the `GroupConfig` interface, default 50, label entry under `Group Play` category); `backend/src/services/admin-game-config.ts` (+~3 lines — mirrored the same field in the `GameConfig` barrel); `backend/src/services/group-bet-flip.ts` (+~25 lines — added `GROUP_BONUS_WAGER_WEIGHT_FALLBACK=50` constant, imported `getGroupConfigKey`, read the weight at the top of `flipGroup()`, and added a new step 7c.2 inside the `withTransaction` block that calls `creditWagering(userId, stake * weight / 100, txQuery)` for every member; wrapped in `if (weight > 0)` so admins can disable the bonus credit entirely); `backend/src/services/bonus.ts` (+~5 lines — updated the doc comment on `creditWagering` to note that groups feed the same per-claim FIFO + user-level counter via this function, with the caller already doing the weighting). All inside the same `withTransaction` SERIALIZABLE block as the payout, so the user-side and house-side legs commit atomically.
  - **Issue/Gap**: The `creditWagering(userId, betAmount)` function in `bonus.ts` is called from `game-engine.ts` after every single-player bet. It never fired for group play. The result was that the `bonus_claims.wagering_completed` and `users.wagering_completed_coins` counters (which drive the bonus-clearing UI) completely ignored group activity. A user with an active bonus claim could clear it by playing solo, but the larger group bets they actually made did not count. The spec mentioned a `bonus_wager_credits` table; that table does not exist. The actual aggregation lives in `users.wagering_completed_coins` (denormalized) and `bonus_claims.wagering_completed` (per-claim FIFO).
  - **Proposed Fix**:
    1. Added a new 26th setting `groupBonusWagerWeight` (default 50%, range 0–100) to `admin-group-config.ts` AND the mirrored `admin-game-config.ts` barrel. The spec was approximate: it used the term 'GROUP_BONUS_WAGER_WEIGHT' but the canonical prefix is `groupBonusWagerWeight` to match the existing 25 group_* settings.
    2. Migration 052 seeds the value at 50 via `INSERT INTO admin_settings (key, value) VALUES ('group_bonus_wager_weight', '50') ON CONFLICT (key) DO NOTHING`. Recorded in `pgmigrations`.
    3. `group-bet-flip.ts` reads the weight via `getGroupConfigKey('groupBonusWagerWeight')` with a hard-coded fallback of 50 if the key is missing. The `if (groupBonusWagerWeightNum > 0)` short-circuit lets admins disable the credit entirely by setting the value to 0.
    4. The credit call iterates over EVERY member (not just winners), so loss-path groups still credit — the user risked the stake regardless of outcome. This matches the existing single-player semantics where `creditWagering` fires for every bet, win or lose.
  - **Live verification (post-deploy, 2026-08-04)**:
    | Scenario | Expected | Got |
    |---|---|---|
    | `admin_settings` row exists | `group_bonus_wager_weight=50` | ✅ confirmed |
    | 1 bonus claim, 1 group flip ($10 stake) | per-claim `wagering_completed` = 5.0 (= 10 × 50%) | ✅ exactly 5.0 |
    | 1 bonus claim, 1 solo bet ($10) on same user | per-claim `wagering_completed` = 10.0 (= 10 × 100%) | ✅ exactly 10.0 |
    | Group vs solo ratio | 5.0 / 10.0 = 0.5 | ✅ exactly 0.5 (matches spec) |
    | User counter `users.wagering_completed_coins` after group flip | matches aggregate of per-claim completed | ✅ confirmed (overwritten by `creditWagering`'s aggregate step which always recomputes from active claims) |
  - **Coverage audit (per "loss path?" prompt)**:
    1. **Win path** (resolve after flip, all members on winning side) → creditWagering for each member. ✅ verified above.
    2. **Loss path** (resolve after flip, members on losing side) → creditWagering for each member (the for-loop iterates over `payouts`, not just winners). ✅ verified by code inspection: `for (const p of payouts) { ... await creditWagering(p.userId, weighted, txQuery); }` — `payouts` includes all members regardless of `isWinner`. Manual flip test on a loss case was attempted but the login rate limit cut off the verification cycle; the code path is provably symmetric (same iteration, same weighted amount).
    3. **Cancel path** (`force-cancel`) → 0 wagering credit. Correct, since the stake is refunded to the user (not lost). Refund does not advance wagering progress because the user didn't actually risk the wager.
    4. **Refund path** (`refund` on resolved room) → 0 wagering credit. Correct, since the original flip's credit (during the resolve) was already applied. Refund reverses the user-side leg, not the house-side accounting.
    5. **Weight=0 admin setting** → no creditWagering call fires. Admins can fully disable the feature.
  - **Known quirk of `creditWagering`'s FIFO** (not a Gap 4 bug, but worth documenting): when a user has multiple active claims, the per-call credit is applied to the OLDEST claim first (`ORDER BY claimed_at ASC`). Damage in a 2-claim test: solo bet credit goes to whichever claim was created first; the newer claim may show 0.0. This is the existing FIFO behavior from `bonus.ts:line 422` and is unchanged by Gap 4. Operators can use `bonus_claim_transfers` (future) to deliberately allocate credits across claims if needed.
